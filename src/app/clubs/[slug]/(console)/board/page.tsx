...
export default async function ClubBoardPage({
  ...
  const pulse = canManageClub
    ? await getBoardPulse(club.id, londonToday())
    : { started: [], active: [] };
  ...
  return (
    <Container maxWidth="md" component="main" sx={{ py: { xs: 4, md: 6 } }}>
      ...
      {/* The club's own view. A board can look busy on page one and have been
          silent since spring, which is the thing an owner wants to know and
          the only thing the list cannot show them. */}
      {canManageClub ? (
        <SectionPulse
          label="Threads over the year"
          labels={pulse.started.map((m) => m.label)}
          series={[
            { name: "Threads started", color: faction.base,
              values: pulse.started.map((m) => m.value) },
            { name: "Threads with activity", color: tokens.brass,
              values: pulse.active.map((m) => m.value) },
          ]}
        />
      ) : null}
