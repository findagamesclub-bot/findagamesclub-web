...
import { getLoyaltyFlow, getProgramme, getStandings, getWallet } from "@/services/loyalty.service";
...
export default async function LoyaltyPage({
  ...
  // A club that runs no programme has no public page. Whoever runs the club
  // still gets one, because Loyalty is in their console and a section listed
  // in a console has to open.
  if (!programme && !canManage) notFound();

  if (!programme) {
    return (
      <Container maxWidth="lg" component="main" sx={{ py: { xs: 4, md: 6 } }}>
        <ClubSectionHeader back={back} title="Loyalty" clubName={club.name}
          clubSlug={club.slug} faction={faction} stats={[]} />
        <EmptyState
          title="No points programme yet"
          description={`${club.name} does not award points. Once it does, what each member has earned and how they earned it shows here.`}
        />
      </Container>
    );
  }
  ...
  // An owner who has not joined their own club has no wallet, so there is no
  // history to show and nothing to sit beside the ladder. Kept as the wallet
  // rather than a flag so the two places that read it stay narrowed.
  const ledger = wallet && membership.status === "approved" ? wallet : null;
  ...
  const flow = canManage ? await getLoyaltyFlow(club.id, londonToday()) : [];
  ...
  return (
    <Container maxWidth="lg" component="main" sx={{ py: { xs: 4, md: 6 } }}>
      ...
      {/* Two columns from md up, and in this order on a phone.
          ...
          One column when there is no history to put beside it. An owner who is
          not a member of their own club has no wallet, so the second column was
          rendering nothing and leaving the ladder half width against a blank
          half page, under a section that ran the full width. */}
      <Box sx={{ display: "grid", gap: { xs: 0, md: 4 }, alignItems: "start",
                 gridTemplateColumns: {
                   xs: "minmax(0, 1fr)",
                   md: ledger ? "minmax(0,1fr) minmax(0,1fr)" : "minmax(0, 1fr)",
                 } }}>
      <Box>
      {/* Issued against spent. A scheme that only ever issues is a liability
          nobody is using, and the two counters are the only way to see it. */}
      {canManage && flow.some((m) => m.issued || m.spent) ? (
        <Section title="Points in and out" icon={ShowChartIcon} navLabel="Points">
          <AreaTrendChart
            labels={flow.map((m) => m.label)}
            series={[
              { name: "Points issued", color: faction.base,
                values: flow.map((m) => m.issued) },
              { name: "Points spent", color: tokens.brass,
                values: flow.map((m) => m.spent) },
            ]}
          />
        </Section>
      ) : null}
      ...
      {ledger ? (
        <Section title="How it adds up" icon={HistoryIcon}>
          <LoyaltyLedger entries={ledger.entries} />
        </Section>
      ) : null}
      </Box>
    </Container>
