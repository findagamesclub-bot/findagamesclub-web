...
import SignOutTextButton from "@/components/layout/SignOutTextButton";
...
import { countByMonth, runningTotal } from "@/utils/club-pulse";
...
export default async function ClubMembersPage({
  ...
  // Worked out from the roster already loaded, so the chart costs no query.
  // `before` is everybody who joined before the run starts, or the line would
  // begin at zero for a club that has been going for years.
  const joinDates = roster.map((m) => m.joinedAt ?? null);
  const inWindow = countByMonth(joinDates, londonToday());
  const rosterTrend = runningTotal(
    inWindow,
    roster.length - inWindow.reduce((n, m) => n + m.value, 0),
  );
  ...
  return (
    <Container maxWidth="lg" component="main" sx={{ py: { xs: 4, md: 6 } }}>
      ...
      {!canSeeRoster ? (
          ...
          {viewer ? (
            <Typography variant="body2" sx={{ color: tokens.inkMuted }}>
              ...
              <SignOutTextButton
                ...
                next={`/auth/sign-in?next=/clubs/${club.slug}/members`}
                sx={{ color: tokens.brand, background: "none", border: 0, p: 0,
        ...
        <>
        {/* A running total rather than who joined each month. A roster only
            goes up, and twelve separate columns hide the shape of that: the
            question an owner has is whether the line is still climbing. */}
        {canManage ? (
          <Box sx={{ mb: 3, p: 2, border: `1px solid ${tokens.rule}`, borderRadius: 1.5,
                     backgroundColor: tokens.paper }}>
            <MonoLabel>How the roster has grown</MonoLabel>
            <AreaTrendChart
              labels={rosterTrend.map((m) => m.label)}
              series={[{ name: "Members", color: faction.base,
                         values: rosterTrend.map((m) => m.value) }]}
            />
          </Box>
        ) : null}
