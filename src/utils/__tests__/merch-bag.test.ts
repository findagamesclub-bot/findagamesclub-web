...
const item = (id: number, name: string, price: string, stock = 10,
  ...
  soldOut: stock === 0, blockedReason: null, variants: [], ...extra,
});
...
// --- lines -----------------------------------------------------------------
assert.deepEqual(addLine([], 1), [{ itemId: 1, variantId: null, quantity: 1 }]);
assert.deepEqual(addLine([{ itemId: 1, variantId: null, quantity: 2 }], 1), [{ itemId: 1, variantId: null, quantity: 3 }]);
assert.deepEqual(addLine([{ itemId: 1, variantId: null, quantity: 1 }], 2, 3),
  [{ itemId: 1, variantId: null, quantity: 1 }, { itemId: 2, variantId: null, quantity: 3 }]);
assert.deepEqual(setQuantity([{ itemId: 1, variantId: null, quantity: 3 }], 1, 0), [], "zero removes the line");
assert.deepEqual(setQuantity([{ itemId: 1, variantId: null, quantity: 3 }], 1, 99), [{ itemId: 1, variantId: null, quantity: 20 }]);
assert.deepEqual(removeLine([{ itemId: 1, variantId: null, quantity: 1 }], 1), []);

// --- pricing ---------------------------------------------------------------
const bag = priceBag({ lines: [{ itemId: 1, variantId: null, quantity: 2 }, { itemId: 2, variantId: null, quantity: 1 }],
...
const withPoints = priceBag({ lines: [{ itemId: 2, variantId: null, quantity: 1 }], items: ITEMS,
...
assert.equal(priceBag({ lines: [{ itemId: 2, variantId: null, quantity: 1 }], items: ITEMS,
...
const noPoints = priceBag({ lines: [{ itemId: 2, variantId: null, quantity: 1 }], items: ITEMS,
...
const odd = priceBag({ lines: [{ itemId: 3, variantId: null, quantity: 3 }], items: [item(3, "Dice", "£19.99")],
...
const quoted = priceBag({ lines: [{ itemId: 6, variantId: null, quantity: 2 }, { itemId: 1, variantId: null, quantity: 1 }],
...
// --- problems --------------------------------------------------------------
const stale = priceBag({ lines: [{ itemId: 4, variantId: null, quantity: 3 }], items: [item(4, "Mat", "£30", 1)],
                         standing: standing(), points: 0 });
assert.equal(stale.lines[0].problem, "Only 1 left");

const gone = priceBag({ lines: [{ itemId: 5, variantId: null, quantity: 1 }], items: [item(5, "Tee", "£10", 0)],
                        standing: standing(), points: 0 });
assert.equal(gone.lines[0].problem, "Sold out since you added it");
assert.equal(gone.blocked, true);

// --- revive ----------------------------------------------------------------
assert.deepEqual(reviveBag([{ itemId: 1, variantId: null, quantity: 2 }, { itemId: 99, variantId: null, quantity: 1 }], ITEMS),
  [{ itemId: 1, variantId: null, quantity: 2 }], "an item the club deleted drops out");
assert.deepEqual(reviveBag("nonsense", ITEMS), []);
assert.deepEqual(reviveBag([{ itemId: 1, quantity: "x" }], ITEMS), [{ itemId: 1, variantId: null, quantity: 1 }]);

console.log("merch-bag ok");

// --- sizes ------------------------------------------------------------------

import { addLine as add2, setQuantity as set2, removeLine as rm2 } from "../merch-bag";

// Two sizes of the same shirt are two lines, not one with a doubled quantity.
const sized = add2(add2([], 1, 1, 10), 1, 1, 11);
assert.equal(sized.length, 2);
assert.deepEqual(sized.map((l) => l.variantId), [10, 11]);

// Adding the same size again bumps that line and leaves the other alone.
const bumped = add2(sized, 1, 2, 10);
assert.equal(bumped.length, 2);
assert.equal(bumped.find((l) => l.variantId === 10)?.quantity, 3);
assert.equal(bumped.find((l) => l.variantId === 11)?.quantity, 1);

// Removing one size leaves the other in the bag.
assert.deepEqual(rm2(sized, 1, 10).map((l) => l.variantId), [11]);
// And setting a size to zero removes only that size.
assert.deepEqual(set2(sized, 1, 0, 11).map((l) => l.variantId), [10]);

// An unsized item still behaves as it always did.
const plain = add2(add2([], 2, 1), 2, 1);
assert.equal(plain.length, 1);
assert.equal(plain[0]!.quantity, 2);

console.log("merch-bag sizes ok");
